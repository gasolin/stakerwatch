...
import useEthTokensBalance from './useEthTokensBalance';

import {i18nValidator, i18nBalance} from '../i18n';

export const EthBalances = ({addresses, fetch}) => {
  if (!addresses) return null;
  const [loadEth, balanceEth] = useNativeTokenBalance(addresses);
  const [loadToken, balanceToken] = useEthTokensBalance(addresses, fetch);
